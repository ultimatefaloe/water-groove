    ...
    return (
      <div className="space-y-3">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="h-16 bg-wg-primary2/50 rounded-lg animate-pulse" />
        ))}
      </div>
    );
  }
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-b-wg-accent/20 hover:bg-transparent">
            <TableHead className="text-wg-primary font-medium">Investment ID</TableHead>
            <TableHead className="text-wg-primary font-medium">User ID</TableHead>
            <TableHead className="text-wg-primary font-medium">Principal</TableHead>
            <TableHead className="text-wg-primary font-medium">ROI Rate</TableHead>
            <TableHead className="text-wg-primary font-medium">Duration</TableHead>
            <TableHead className="text-wg-primary font-medium">Status</TableHead>
            <TableHead className="text-wg-primary font-medium">Start Date</TableHead>
        ...
        <TableBody>
          {investments.map((investment) => {
            ...
            return (
              <TableRow key={investment.id} className="border-b-wg-accent/10 hover:bg-wg-primary/10">
                <TableCell className="font-medium text-wg-primary">
                  <div className="text-sm">{investment.id.substring(0, 8)}...</div>
                ...
                <TableCell className="text-wg-primary/80">
                  <span className="font-medium">{(investment.roiRateSnapshot * 100).toFixed(2)}%</span>
                  <div className="text-xs text-wg-primary/60">
                    ROI: {calculateROI(investment.principalAmount, investment.roiRateSnapshot, investment.durationMonths)}
                  </div>
                </TableCell>
                <TableCell className="text-wg-primary/80">
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    {isAdmin && (
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent 
                          align="end" 
                          className="bg-wg-primary2 border-wg-accent/20 w-48"
                        >
                          ...
                          {investment.status === 'ACTIVE' && (
                            <DropdownMenuItem 
                              onClick={() => onAction(investment, 'PAUSE')}
                          ...
                          {investment.status === 'PAUSED' && (
                            <DropdownMenuItem 
                              onClick={() => onAction(investment, 'RESUME')}
                              className="text-green-400 hover:bg-green-500/20 cursor-pointer"
                            >
                          ...
                          {(investment.status === 'ACTIVE' || investment.status === 'PAUSED' || investment.status === 'PENDING_PAYMENT') && (
                            <DropdownMenuItem 
                              onClick={() => onAction(investment, 'CANCEL')}
                          ...
                          <DropdownMenuSeparator className="bg-wg-accent/20" />
                          <DropdownMenuItem 
                            onClick={() => onAction(investment, 'FORCE_COMPLETE')}
